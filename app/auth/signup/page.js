...
export default function SignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    setLoading(false);
    if (!data.success) { setMessage(data.message || "Signup failed"); return; }
    ...
    <div className="relative min-h-screen bg-[#faf9ff] flex flex-col items-center justify-center px-4 py-10 overflow-hidden font-sans">
      ...
      <div className="w-full max-w-[420px] bg-white/85 backdrop-blur-3xl border border-violet-100 rounded-[30px] px-8 py-8 shadow-[0_16px_64px_rgba(109,93,230,0.13)] animate-[fadeUp_0.6s_ease_both] [animation-delay:0.1s]">
        ...
        <div className="flex gap-1 bg-[#f3f1ff] rounded-2xl p-1 mb-7">
          <Link href="/auth/login" className="flex-1 text-center py-2.5 rounded-xl text-[13px] font-semibold text-gray-400 hover:text-violet-500 transition-colors">
            Sign In
          </Link>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="animate-[fadeUp_0.45s_ease_both] [animation-delay:0.18s]">
            ...
            <div className="relative flex items-center">
              <User size={14} className="absolute left-3.5 text-violet-300 pointer-events-none" />
              <input
                type="text" name="name" placeholder="Ani Joy"
          ...
          {/* Password */}
          <div className="animate-[fadeUp_0.45s_ease_both] [animation-delay:0.32s]">
            ...
            <div className="relative flex items-center">
              ...
              <input
                type={showPassword ? "text" : "password"} name="password" placeholder="Make it a strong one"
          ...
          <button type="submit" disabled={loading}
            className="animate-[fadeUp_0.45s_ease_both] [animation-delay:0.38s] relative w-full py-3.5 mt-0.5 bg-gradient-to-br from-violet-600 via-violet-500 to-indigo-500 text-white font-extrabold rounded-2xl text-[14px] tracking-tight shadow-[0_8px_28px_rgba(109,93,230,0.38)] hover:shadow-[0_10px_32px_rgba(109,93,230,0.48)] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden group"
          >
            <span className="relative z-10">{loading ? "Creating account..." : "Start exploring →"}</span>
        ...
        <p className="text-center text-[12px] text-gray-400 mt-6">
          ...
          <Link href="/auth/login" className="text-violet-500 font-extrabold hover:text-violet-700 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
